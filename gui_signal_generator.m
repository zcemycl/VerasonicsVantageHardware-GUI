...
% This function helps to obtain the latest values from the workspace that 
% are useful for plotting 
function [TPC, number_cycles, pulse_repetition, frequency, ...
        initialize, txapod, txdelay, xmax,polarity] = loadworkspace
    TPC = evalin('base','TPC');
...
[TPC, number_cycles, pulse_repetition, frequency, initialize, ...
    txapod, txdelay, xmax, polarity] = loadworkspace;
...
% General Settings
% ======================================================================= %
% Create frame to hold general settings which contain
% frequency, number of cycles, pulse repetition and polarity
% ======================================================================= %
% Frame
...
poltext = uicontrol('Parent',f,...
    ...
    'FontUnits','normalized',...
    'FontSize',0.4);
% Toggle Button: press --> positive; unpress --> negative
...
save_gui_button = uicontrol('Parent',f,...
    ...
    'FontUnits','normalized',...
    'FontSize',0.4,...
    'Position',[0.82,0.39,0.15,0.05],...
    'Callback',{@save_gui_callback});
% ======================================================================= % 
% Individual Settings
% Create frame to hold individual settings which contain
% apodization, on/off and delay
% ======================================================================= % 
% Frame
...
% ====================================================================== %
% Callbacks for individual settings
% ====================================================================== %
% ----------------------
% ON OFF BUTTONS
% ----------------------
...
function turn_element_off(toggle_handle, element_number) 
    % Obtain all the useful parameters for plotting
    [TPC, number_cycles, pulse_repetition, frequency, ...
        initialize, txapod, txdelay, xmax, polarity] = loadworkspace;
    % Targets to be updated later
    initialize = evalin('base','initialize');
    TX = evalin('base','TX');
    % ================================================================ %
    % UI input
    ...
    % Condition
    % ================================================================ %
    if onoff > 0 % Pressed --> update the system and plot the waveform
    ...
    else % Unpressed --> update the system and erase the waveform in the plot
        % Change the word in the text
...
function v1edit_callback(hObject,eventdata)
    ...
    % Other actions : obtain the latest parameters for plotting
    [TPC, number_cycles, pulse_repetition, frequency,...
        initialize, txapod, txdelay, xmax, polarity] = loadworkspace;
    % Other actions : plotting
...
% function to group the callbacks content of voltage
...
function drivedelay(delayedit_handle, element_number)
    ...
    delay_in_wavelength = delay*Trans.frequency*1e6;    
    txdelay = evalin('base','txdelay');
...
% ====================================================================== %
% Callbacks for general settings
% ====================================================================== %
% Frequency
% ====================================================================== %
...
% imaginery data for plotting
    ...
    Trans = evalin('base','Trans');
% Plot the curves if the togglebutton is on, delete the curves if off
% ** The use of Trans.frequency (constant) is important, if it is replaced by
% frequency, the curves' delays vary depending on the changing frequency
    hold on;
    if i == 1 && initialize(1) == 1
    delete(findobj('Tag','Plot1'));
    p1 = plot(linspace(txdelay(1)/Trans.frequency,txdelay(1)/Trans.frequency+number_cycles*1/frequency ,100*number_cycles),-polarity*txapod(1)*TPC(1).hv*y,'r','Tag','Plot1',...
                'DisplayName','Channel 1');
    elseif i == 2 && initialize(2) == 1
    delete(findobj('Tag','Plot2'));
    p2 = plot(linspace(txdelay(2)/Trans.frequency,txdelay(2)/Trans.frequency+number_cycles*1/frequency ,100*number_cycles),-polarity*txapod(2)*TPC(1).hv*y,'g','Tag','Plot2',...
                'DisplayName','Channel 2');
    elseif i == 3 && initialize(3) == 1
    delete(findobj('Tag','Plot3'));
    p3 = plot(linspace(txdelay(3)/Trans.frequency,txdelay(3)/Trans.frequency+number_cycles*1/frequency ,100*number_cycles),-polarity*txapod(3)*TPC(1).hv*y,'b','Tag','Plot3',...
                'DisplayName','Channel 3');
    elseif i == 4 && initialize(4) == 1
    delete(findobj('Tag','Plot4'));
    p4 = plot(linspace(txdelay(4)/Trans.frequency,txdelay(4)/Trans.frequency+number_cycles*1/frequency ,100*number_cycles),-polarity*txapod(4)*TPC(1).hv*y,'k','Tag','Plot4',...
...
% If UI input > upper limit, UI input = upper limit
% If UI input < lower limit, UI input = lower limit
