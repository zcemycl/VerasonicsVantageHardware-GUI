...
% starting values for signal output (all in normal units)
...
PARAMETRIC_HALF_CYCLE_ON_TIME_FRAC  = 0.67; % to simulate the sine waves


% upper limit and lower limit of each edit text
% will be used for returning the number back to extreme value
% if sth > max , sth --> max
% if sth < min , sth --> min
% Number of cycles
ncpmax                              = 20 ; 
...
% ---------------------------------------------------------------------- %
% start with all elements turned off, this should only contain 
% the 1/0 value for each element
